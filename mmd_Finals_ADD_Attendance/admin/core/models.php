...
function getAllAttendancesByDate($pdo, $date_today) {
	$sql = "SELECT 
				...
				GROUP_CONCAT(timestamp_record_added ORDER BY timestamp_record_added SEPARATOR ' :: ') AS in_and_out  -- aggregate different rows from same column
			...
			GROUP BY attendance_records.user_id  -- separate aggregated timestamp_records by user
			ORDER BY 
...
function updateLeaveStatus($pdo, $status, $leave_id) {
	$sql = "UPDATE leaves SET status = ?, admin_seen = true, employee_seen = false WHERE leave_id = ?";
	$stmt = $pdo->prepare($sql);
...
function numberOfUnreadAdmin($pdo) {
	$sql = "SELECT COUNT(admin_seen) AS unread_count FROM leaves WHERE admin_seen = 0";
	$stmt = $pdo->prepare($sql);
	$stmt->execute();
    $row = $stmt->fetch();
