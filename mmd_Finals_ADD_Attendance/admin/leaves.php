      ...
      <div class="row justify-content-center">
        <div class="col-md-8 mt-4" style="font-family: 'Arial';">
            ...
            <?php foreach ($getAllLeaves as $row) { ?>
              ...
              <!-- add data attribute to div based on leave request status -->
              <div class="leaveReq card shadow mt-4"
                <?php if ($row['admin_seen']) { ?>
      ...
      $('.leaveReq').each(
        function(event) {
          ...
          switch ($(this).data('read-status')) {
            case 'seen':
              $(this).find('.fullName').removeClass('myBold').append(' <span style="font-size: 18px; color: #9CA3AF;">(seen)</span>');
              break;
            
            case 'unseen':
              $(this).find('.fullName').addClass('myBold').append(' <span style="font-style: italic;">(new)</span>');
              break;
        ...
        if (formData.leave_id != "" && formData.leave_status != "") {
          $.ajax({
            ...
            success: function (data) {
              $(this).closest('.leaveReq').attr('data-leave-status', 'seen');
              location.reload();
            }
          })
        }
