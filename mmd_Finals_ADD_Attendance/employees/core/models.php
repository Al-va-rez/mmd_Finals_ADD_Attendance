...
function insertNewLeave($pdo, $description, $user_id, $date_start, $date_end) {
	$sql = "INSERT INTO leaves (description, user_id, date_start, date_end, admin_seen, employee_seen) 
			VALUES (?,?,?,?, false, true)";
	$stmt = $pdo->prepare($sql);
...
function updateLeaveDescription($pdo, $description, $date_start, $date_end, $leave_id) {
	$sql = "UPDATE leaves SET description = ?, date_start = ?, date_end = ?, admin_seen = false, employee_seen = true 
			WHERE leave_id = ?";
	$stmt = $pdo->prepare($sql);
...
// functions for notification feature
function updateReadStatus($pdo, $leave_id) {
	$sql = "UPDATE leaves SET employee_seen = true WHERE leave_id = ?";
	$stmt = $pdo->prepare($sql);
	return $stmt->execute([$leave_id]);
}

function numberOfUnreadEmployee($pdo, $user_id) {
	$sql = "SELECT COUNT(employee_seen) AS unread_count FROM leaves WHERE employee_seen = false AND user_id = ?";
	$stmt = $pdo->prepare($sql);
	$stmt->execute([$user_id]);
    $row = $stmt->fetch();
