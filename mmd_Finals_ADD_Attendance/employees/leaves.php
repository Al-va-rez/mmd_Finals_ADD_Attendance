      ...
      <div class="row justify-content-center">
        ...
        <div class="col-md-10">
            ...
            <?php foreach ($getLeavesByUserId as $row) { ?>
              <div class="leaveReq card shadow mt-4"
                <?php if ($row['employee_seen']) { ?>
                  data-read-status="seen"
                <?php } else { ?>
                  data-read-status="unseen"
                <?php } ?>
              >
                <div class="card-body p-5 leaveContainer">
                  ...
                  <!-- <input type="hidden" id="leave_id_copy" value="<?php echo $row['leave_id']; ?>"> -->
                  <button class="markAsSeenBtn">Mark as seen</button>
    ...
    <script>
      ...
      $('.leaveReq').each(
        function(event) {
          ...
          switch ($(this).data('read-status')) {
            case 'seen':
              $(this).find('.leaveStatus').removeClass('myBold').append(' <span style="font-size: 14px; color: #9CA3AF;">(seen)</span>');
              $(this).find('.markAsSeenBtn').hide();
              break;
            
            case 'unseen':
              $(this).find('.leaveStatus').addClass('myBold').append(' <span style="font-style: italic;">(request updated)</span>');
              $(this).find('.markAsSeenBtn').show();
              break;
          
            default:
              $(this).find('.leaveStatus').removeClass('myBold');
              $(this).find('.markAsSeenBtn').hide();
      ...
      $('.markAsSeenBtn').each(
        function(event) {
          $(this).on('click',
            function(event) {
              var clickData = {
                leave_id: $(this).closest('.leaveReq').find('#leave_id').val(),
                markAsSeen: 1
              };
